...
let responseContents: {message:string, statusCode:number} = {message :"Unknown error occured",  statusCode:500}
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  transporter.sendMail(mailData, function (err: any, info: any) {
      if (err) {
          ...
          console.log(info)
          responseContents = { message: info, statusCode: 200 }
      }
  })

   res.status(responseContents.statusCode).json(responseContents.message)
